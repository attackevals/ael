...
In order to request a service ticket for an account using an enterprise principal (i.e. _<user@domain.com>_), the `/enterprise` flag can be used.
...
If the `/enterprise` flag is used, the spn is assumed to be an enterprise principal (i.e. _<user@domain.com>_). This flag only works when kerberoasting with a TGT.
