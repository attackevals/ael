...
def is_admin():
    ...
    try:
        is_admin_check: bool = os.getuid() == 0
...
def validate_root_directory(path):
    ...
    if not validate_path(path):
        if not create_directory(path):
            print(
                "[!] The supplied directory ({0}) is not valid, or was not created. Exiting.".format(
                    path,
                ),
            )
            sys.exit(1)
...
def create_directory(path):
    ...
    if PROMPT:
        print(
            "\nThe supplied directory does not exist. Would you like to create it? ",
            end="(Y/n): ",
        )
        user_input = input()
    ...
    if user_input.lower() == "y" or not PROMPT:
        try:
            ...
            print(
                "[!] Something went wrong when creating {0}:".format(
...
def validate_template_directory(path):
    ...
    if not validate_path(path):
        print(
            "[!] FATAL - The template directory ({0}) was not found. Exiting.".format(
                os.path.abspath(path),
            ),
        )
        sys.exit(1)
    ...
    if not validate_templates(path):
        print(
            "[!] FATAL - Could not find the expected templates in the supplied folder ({0}). Exiting.".format(
                os.path.abspath(path),
            ),
        )
        sys.exit(1)
...
def distribute(base_path, template_folder, file_count):
    ...
    try:
        ...
        print(
            "|--[!] FATAL - Unable to get the child directories of {0}: Permissions error. Exiting.".format(
                os.path.abspath(base_path),
            ),
        )
        sys.exit(1)
    print(
        "|--[*] Found {0} sub-directories in base path".format(
            str(len(child_directories)),
        ),
    )
    ...
    for child_dir in child_directories:
        print("|\n|--[*] Using directory: {0}".format(child_dir))
        try:
            sub_directories = get_child_directories(child_dir)
        except PermissionError:
            print(
                "|--[!] Unable to get the child directories of {0}. Skipping.".format(
                    child_dir,
                ),
            )
            continue

        # if function cannot create or write to either Documents and Desktop, try to write to base folder instead
        ...
        for folder in USER_FOLDERS:
            ...
            try:
                ...
                write_count = copy_files(
                    folder_path,
                    (math.floor(files_per_folder / 2)),
                    templates,
                )
        ...
        if use_base_directory:
            try:
                print(
                    "|----[*] Could not write to Documents or Desktop. Trying base path.",
                )
                print("|----[*]Copying files to {0}".format(child_dir))
                write_count = copy_files(child_dir, files_per_folder, templates)
                print("|----[OK] Wrote {0} files to directory".format(write_count))
            except IOError:
                print("|----[!] Failed to write to {0}:".format(folder_path))
...
def get_templates(template_folder):
    ...
    """
    files = {
        os.path.splitext(f)[1]: os.path.abspath(f)
        for f in os.scandir(template_folder)
...
def main():
    parser = argparse.ArgumentParser(
        description="Distribute template files for testing. Takes a templates folder and "
    ...
    parser.add_argument(
        "-c",
        "--count",
        help="The total number of files to distribute. Default: 100",
        type=int,
        default=100,
    )
    parser.add_argument(
        "-s",
        "--seed",
        help="String seed for random file name generation. "
        + "Do not use 'none' as that will cause no seed to be used.",
        type=str,
    )
    parser.add_argument(
        "--noprompt",
        help="Causes the program to not prompt for file creation permission.",
        action="store_false",
    )
    ...
    if not is_admin():
        print(
            "[!] To avoid issues dispersing files, please run this with elevated privileges. Exiting.",
        )
        sys.exit(1)
    else:
        print(
            "[OK] Running as {0}".format(
                "Administrator" if OS == "Windows" else "root",
            ),
        )
