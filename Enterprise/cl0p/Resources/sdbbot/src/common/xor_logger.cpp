 ...
 * To use this file, include it and xor_logger.hpp in your implant source code, adjust DEBUG_MODE if desired,
 * adjust the Base64 encoder include path in xor_logger.cpp, and call xor_logger::InitializeLogger from your main
...
// Only check and create the log file if compiled for debug
    ...
    /// HELPER FUNCTIONS
#ifdef DEBUG
    int appendStringToLog(std::string data) {
        // Critical section
...
#ifdef DEBUG
        std::string encrypted_encoded = encryptAndEncodeLogMsg(formatted);
        return appendStringToLog(encrypted_encoded);
#else
        return 0;
#endif
    }
