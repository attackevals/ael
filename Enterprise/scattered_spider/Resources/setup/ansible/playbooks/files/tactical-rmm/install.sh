...
wget -qO - https://nginx.org/keys/nginx_signing.key | sudo gpg --batch --yes --dearmor -o /etc/apt/keyrings/nginx-archive-keyring.gpg
...
wget --quiet -O - https://www.postgresql.org/media/keys/ACCC4CF8.asc | sudo gpg --batch --yes --dearmor -o /etc/apt/keyrings/postgresql-archive-keyring.gpg
sudo apt update
...
printf >&2 "\n"
echo -ne "Username: "
if [[ -n $DJANGO_USERNAME ]]; then
	djangousername=$DJANGO_USERNAME
else
	read djangousername
fi
export DJANGO_SUPERUSER_PASSWORD=$DJANGO_PASSWORD
python manage.py createsuperuser --username ${djangousername} --email ${letsemail} --noinput
python manage.py create_installer_user
RANDBASE=$(python manage.py generate_totp)
cls
python manage.py generate_barcode ${RANDBASE} ${djangousername} ${frontenddomain} &>/tmp/output.log
deactivate
