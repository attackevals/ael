...
func clearLogFile(t *testing.T, handle *os.File) {
	if err := CloseLogFile(handle); err != nil {
		t.Error(err)
	}
...
func verifyLogFile(t *testing.T, prefix string, want []string) {
	...
	if len(want) != len(contents) {
		...
		for index, line := range(contents) {
			toCheck := line[offset:]
			assert_util.AssertEq(t, toCheck, want[index], "")
			prefixCheck := line[:prefixLen]
			assert_util.AssertEq(t, prefixCheck, prefix, "")
		}
	}
}
...
func TestDebug(t *testing.T) {
	handle := SetLogs(TEST_LOG_FILENAME)
	defer clearLogFile(t, handle)
