...
// Creates and adds the SimpleFileServerHandler C2 handler to the map of available C2 handlers.
func init() {
    util.AvailableHandlers["simplefileserver"] = simpleFileServerHandlerFactory(&sync.WaitGroup{})
}
...
func (s *SimpleFileServerHandler) handleFileUpload(w http.ResponseWriter, r *http.Request) {
	...
	response, err := util.ForwardFileUpload(s.restAPIaddress, fileName, fileData)
    if err != nil {
