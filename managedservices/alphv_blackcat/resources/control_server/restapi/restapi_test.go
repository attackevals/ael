...
func startRESTapi(testConfigPath string) {
    ...
    configPath := "./config/test_restapi_config.yml" // default config path if none specified
...
func TestUpdateSession(t *testing.T) {
    ...
    updateSession, err := json.Marshal(map[string]interface{}{"user": "updatedUser", "hostName": "updatedHost", "pid": 9999})
    // setup HTTP POST request
    url := BASE_URL + "session/update/" + TEST_SESSION_1_GUID
    req, err := http.NewRequest("POST", url, bytes.NewBuffer(updateSession))
    if err != nil {
        t.Error(err)
    }
    req.Header.Set("Content-Type", "application/json")
...
func TestGetFile(t *testing.T) {
    ...
    url := BASE_URL + "files/hello_world.pdf"
    resp, err := http.Get(url)
    if err != nil {
        t.Error(err)
    }
    ...
    // MD5 hash of control_server/files/hello_world.pdf
    expectedMD5 := "ff1665c2e7239b61f8d3f22248a4bff8"
...
func TestUploadFile(t *testing.T) {
    ...
    // read file
    fileData, err := ioutil.ReadFile("test_payloads/hello_world.pdf")
    if err != nil {
        t.Error(err)
    }

    // upload file via HTTP POST
    url := BASE_URL + "upload/test_file.pdf"
  ...
  "type": 0,
  "status": 0,
  "data": "Successfully uploaded file to control server at './files/test_file.pdf'"
}
`
    if string(body) != expectedOutput {
        t.Errorf("Expected '%v' got '%v'", expectedOutput, string(body))
    ...
    uploadedTestFile := "./files/test_file.pdf"
