...
resource "aws_organizations_account" "protections" {
  provider          = aws.cloud
  parent_id         = data.aws_organizations_organizational_unit.cloud-ou-vendors.id
  name              = "enterpriseround7-protections"
  email             = var.account-protections-email
...
# [PROTECTIONS] Assign account to OrgAdmins
resource "aws_ssoadmin_account_assignment" "assign-to-protections-orgadmins" {
  provider           = aws.cloud
  ...
  target_id   = aws_organizations_account.protections.id
