...
func UpdateSession(guid string, updates map[string]string) (Session, error) {
    ...
    for name, val := range updates {
        switch name {
        case "ipAddr":
            SessionList[index].IPAddr = val
        case "hostName":
            SessionList[index].HostName = val
        case "user":
            SessionList[index].User = val
        case "dir":
            SessionList[index].Dir = val
        case "pid":
            SessionList[index].PID = val
        case "ppid":
            SessionList[index].PPID = val
        }
    }
