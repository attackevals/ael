...
namespace Quasar.Server.Forms
{
    public partial class FrmMain : Form
    {
        ...
        public FrmMain(bool initialize)
        {
        ...
        private void InitializeServer()
        {
            ...
            Logger.Debug("Current working directory: " + Directory.GetCurrentDirectory());
            Logger.Debug("Application startup path: " + Application.StartupPath);
#if DEBUG
...
#endif
            // Certificate and options logging
            LogCertDetailsAndOptions();
        ...
        private void LogCertDetailsAndOptions()
        {
            var serverCertToLog = new X509Certificate2(Settings.CertificatePath, "", X509KeyStorageFlags.Exportable);
            var key = serverCertToLog.Thumbprint;

            byte[] signature;
            // https://stackoverflow.com/a/49777672 RSACryptoServiceProvider must be changed with .NET 4.6
            using (var csp = (RSACryptoServiceProvider) serverCertToLog.PrivateKey)
            {
                var hash = Sha256.ComputeHash(Encoding.UTF8.GetBytes(key));
                signature = csp.SignHash(hash, CryptoConfig.MapNameToOID("SHA256"));
            }
            Logger.Debug("Quasar Options:");
            Logger.Debug(String.Format("Encryption key: {0}", key));

            Logger.Debug("Server Cert (base64):");
            Logger.Debug(Convert.ToBase64String(serverCertToLog.Export(X509ContentType.Cert))); // export without private key
            Logger.Debug("Server Cert Signature (base64):");
            Logger.Debug(Convert.ToBase64String(signature));
        }
        ...
        public virtual Client[] GetConnectedClients()
        {
            return ListenServer.ConnectedClients;
        }

        /// <summary>
        /// Returns a currently connected and identified client belonging to the given client Id, or null if not found.
        /// </summary>
        public virtual Client LookupClientById(string clientId)
        {
            return ListenServer.LookupClientById(clientId);
        }
