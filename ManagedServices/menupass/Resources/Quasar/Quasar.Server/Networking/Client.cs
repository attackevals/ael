...
namespace Quasar.Server.Networking
{
    ...
    {
        /// <summary>
        /// Unique ID for the client. Equal to MD5 hash of client hostname + username + user account type.
        /// </summary>
        ...
        /// <summary>
        /// Checks whether the clients are equal based on ID comparison.
        /// </summary>
        ...
        public Client(BufferPool bufferPool, SslStream stream, IPEndPoint endPoint)
        {
            try
            {
                ...
                if (stream == null || bufferPool == null)
                {
                    Logger.Error("Null stream or buffer pool for client.");
                    Disconnect();
                }
                _stream = stream;
            ...
            {
                Logger.Error(String.Format("Exception when creating Client object: {0}", ex.ToString()));
                Disconnect();
            }
        }
        ...
        private void AsyncReceive(IAsyncResult result)
        {
            ...
            {
                Logger.Error(String.Format("AsyncReceive: exception when reading stream: {0}", ex.ToString()));
                Disconnect();
                return;
            }
            ...
            {
                Logger.Error(String.Format("AsyncReceive: exception when copying received buffer: {0}", ex.ToString()));
                Disconnect();
                return;
            }
                        ...
                        case ReceiveType.Header:
                            {
                                ...
                                if (_readableDataLen + _writeOffset >= HeaderSize)
                                {
                                    ...
                                    {
                                        process = false;
                                        Logger.Error(String.Format("AsyncReceive: exception when handling complete header: {0}", ex.ToString()));
                                        Disconnect();
                                        break;
                                    }
                                ...
                                else // _readableDataLen + _writeOffset < HeaderSize
                                {
                                    ...
                                    {
                                        process = false;
                                        Logger.Error(String.Format("AsyncReceive: exception when handling partial header: {0}", ex.ToString()));
                                        Disconnect();
                                        break;
                                    }
                        ...
                        case ReceiveType.Payload:
                            {
                                ...
                                {
                                    process = false;
                                    Logger.Error(String.Format("AsyncReceive: exception when handling payload receiveType: {0}", ex.ToString()));
                                    Disconnect();
                                    break;
                                }
                                ...
                                if (_writeOffset - HeaderSize == _payloadLen)
                                {
                                    ...
                                    {
                                        process = false;
                                        Logger.Error(String.Format("AsyncReceive: exception when handling complete payload receiveType: {0}", ex.ToString()));
                                        Disconnect();
                                        break;
                                    }
        ...
        private void SafeSendMessage(IMessage message)
        {
            ...
            {
                Logger.Error(String.Format("SafeSendMessage: exception when handling payload: {0}", ex.ToString()));
                Disconnect();
                SendCleanup(true);
            }
            finally
