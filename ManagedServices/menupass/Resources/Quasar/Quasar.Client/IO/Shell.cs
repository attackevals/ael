...
namespace Quasar.Client.IO
{
    ...
    public class Shell : IDisposable
    {
        ...
        private void CreateSession()
        {
            ...
            RedirectIO();
            Logger.Debug("Started CMD shell session.");
        ...
        private void RedirectStandardOutput()
        {
            ...
            {
                Logger.Error("RedirectStandardOutput exception in shell session: " + ex.ToString());
                if (ex is ApplicationException || ex is InvalidOperationException)
                {
                    ...
                    Logger.Error("Session unexpectedly closed. Starting new session.");
                    CreateSession();
                }
            }
        }
        ...
        private void RedirectStandardError()
        {
            ...
            {
                Logger.Error("RedirectStandardError exception in shell session: " + ex.ToString());
                if (ex is ApplicationException || ex is InvalidOperationException)
                {
                    _client.Send(new DoShellExecuteResponse
                    {
        ...
        public bool ExecuteCommand(string command)
        {
            ...
            if (_prc == null || _prc.HasExited)
            {
                try
                {
                    Logger.Debug("Creating new session to execute command");
                    CreateSession();
                }
                catch (Exception ex)
                {
                    Logger.Error("Failed to create shell session to execute command: " + ex.ToString());
                    _client.Send(new DoShellExecuteResponse
                    {
                        Output = $"\n>> Failed to create shell session: {ex.Message}\n",
                        IsError = true
                    });
                    return false;
                }
            ...
            _inputWriter.Flush();
            Logger.Debug("Delivered command to session.");
