...
func TestUpdateSession(t *testing.T) {
    ...
    if session.HostName != updates["hostName"] {
        t.Errorf("Expected return value of %v, got %v\n", updates["hostName"], session.HostName)
    }
    if session.User != updates["user"] {
        t.Errorf("Expected return value of %v, got %v\n", updates["user"], session.User)
    }
    if session.Dir != updates["dir"] {
        t.Errorf("Expected return value of %v, got %v\n", updates["dir"], session.Dir)
    }
    if float64(session.PID) != updates["pid"] {
        t.Errorf("Expected return value of %v, got %v\n", updates["pid"], session.PID)
    }
    if float64(session.PPID) != updates["ppid"] {
        t.Errorf("Expected return value of %v, got %v\n", updates["ppid"], session.PPID)
    }
}
