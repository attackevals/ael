...
func UpdateSession(guid string, updates map[string]interface{}) (Session, error) {
    valid, index := SessionExists(guid)
    if !valid {
        var session Session
        return session, fmt.Errorf("unable to update session: %v", guid)
    }
    for name, val := range updates {
        switch name {
        case "ipAddr":
            SessionList[index].IPAddr = val.(string)
        case "hostName":
            SessionList[index].HostName = val.(string)
        case "user":
            SessionList[index].User = val.(string)
        case "dir":
            SessionList[index].Dir = val.(string)
        case "pid":
            SessionList[index].PID = int(val.(float64))
        case "ppid":
            SessionList[index].PPID = int(val.(float64))
        }
    }
    return SessionList[index], nil
}
